...
export const AdatProvider = ({children}) => {
    ...
    const getLista = async (vegpont, callBack)=> {//végpontról adatokat kérünk le
    ...
    const postData = async ({...adat},vegpont)=> {
        ...
        catch (error){//ha hálózati hiba van
            console.log(error)

        }
    }

    const deleteData = async (id)=> {//törlés id alapján
        try {
    ...
    return (
        ...
        <AdatContext.Provider
            value={{lista,setLista,postData,deleteData}}
        >
            {children}
